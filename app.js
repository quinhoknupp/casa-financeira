...
function loadState() {
  ...
  if (parsed.version !== DATA_VERSION) {
    localStorage.setItem("casa-financeira-state", JSON.stringify(initialState));
...
function syncDemo() {
  const now = new Date();
  const nextId = state.transactions.length ? Math.max(...state.transactions.map((tx) => tx.id)) + 1 : 1;
...
function resetApp() {
  ...
  state.toast = "Tudo zerado. Pronto para começar do zero.";
  saveState();
  render();
}
    ...
    <main class="shell">
      ...
      <section class="hero">
        <article class="summary">
          ...
          <button class="reset-link" onclick="resetApp()">Zerar dados</button>
        </article>
